...
const navidromeUtils = {
    createMediaObject<T extends MediaObject>(
        ...
        switch (itemType) {
            ...
            case ItemType.Artist:
                return this.createMediaArtist(item as Navidrome.Artist, childSort) as T;

            case ItemType.Playlist:
                return this.createMediaPlaylist(item as Navidrome.Playlist, childSort) as T;
    ...
    createMediaItem(song: Navidrome.Song): MediaItem {
    ...
    createRadioItem(radio: Navidrome.Radio): MediaItem {
        return {
            itemType: ItemType.Media,
            mediaType: MediaType.Audio,
    ...
    createMediaAlbum(album: Navidrome.Album): MediaAlbum {
    ...
    createMediaArtist(artist: Navidrome.Artist, albumSort?: SortParams): MediaArtist {
        ...
        mediaArtist.pager = this.createArtistAlbumsPager(mediaArtist as MediaArtist, albumSort);
        return mediaArtist as MediaArtist;
    },

    createMediaPlaylist(playlist: Navidrome.Playlist, itemSort?: SortParams): MediaPlaylist {
    ...
    createThumbnail(id: string, width: number, height = width): Thumbnail {
        const {host} = navidromeSettings;
        const url = `${host}/rest/getCoverArt?id=${id}&size=${width}&{navidrome-credentials}`; // not a typo
        return {url, width, height};
    },

    createArtistAlbumsPager(artist: MediaArtist, albumSort?: SortParams): Pager<MediaAlbum> {
        const id = this.getMediaObjectId(artist);
    ...
    createArtistAllTracks(artist: MediaArtist): MediaAlbum {
        const id = this.getMediaObjectId(artist);
        return {
            itemType: ItemType.Album,
    ...
    createAllTracksPager(artist: MediaArtist): Pager<MediaItem> {
        const id = this.getMediaObjectId(artist);
    ...
    createPlaylistItemsPager(playlist: MediaPlaylist, itemSort?: SortParams): Pager<MediaItem> {
        const id = this.getMediaObjectId(playlist);
    ...
    getMediaObjectId(object: MediaObject): string {
